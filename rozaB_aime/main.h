...
VOID LineGouraud(int x1, int x2, int y, double z1, double z2, double k1, double k2, COLORREF clr)
{
	...
	if (x1 == x2)
	{
		SetPixelZ((int)x1, (int)y, (float)z1, 0);
		return;
	}
	...
	if (x1 < x2)
		for (x = x1+1; x < x2; x++)
		{
			z = z1 + (double)(x - x1)*kz;
			k = k1 + (double)(x - x1)*kk;
			SetPixelZ(x, y, z, RGB((BYTE)((double)k*red),
				(BYTE)((double)k*gre),
				(BYTE)((double)k*blu)));
		}
	...
	SetPixelZ(x1, y, (double)z1, 0);
	SetPixelZ(x2, y, (double)z2, 0);
}
...
VOID PolygonGouraud(VERTEX *v, int nv, COLORREF clr)
{
	...
	for (y = minY; y <= maxY; y++)
	{
		...
		{
			if (x[0] <= x[1])
				LineGouraud((int)x[0], (int)x[1], (int)y, z[0], z[1], 1, 0.5, clr);
			else
				LineGouraud((int)x[1], (int)x[0], (int)y, z[1], z[0], 1, 0.5, clr);
		}
	}
}
