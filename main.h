...
VOID Surface(HDC hdc, int nx, int ny, int dx, int dy)
{
	...
	for (indx = -nx; indx <= nx; indx += dx)
	{
		...
		y = Y(indy);
		z1 = Cosc(x, y);
		ScreenCoords(pt[0], x, y, z1);
		x = X(indx + dx);
		y = Y(indy);
		z2 = Cosc(x, y);
		ScreenCoords(pt[1], x, y, z2);
		x = X(indx + dx);
		y = Y(indy + dy);
		z3 = Cosc(x, y);
		ScreenCoords(pt[2], x, y, z3);
		x = X(indx);
		y = Y(indy + dy);
		z4 = Cosc(x, y);
		ScreenCoords(pt[3], x, y, z4);


		avgZ =  max(0,((z1 + z2 + z3 + z4 -4.0*minZ) / 4.0)*256);
		NewBrush = CreateSolidBrush(RGB((int)avgZ, (int)avgZ, (int)avgZ));
